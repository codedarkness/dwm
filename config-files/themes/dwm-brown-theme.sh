...
# -----------------------------------------------------------------
#
#        FILE: dwm-browm-theme.sh
#       USAGE: ./dwm-browm-theme.sh
#
...
echo "Changing color scheme for dwm - Brown"

echo "-------------------------------------------------------------"

## background color
sed -i 's/col_gray1\[\]\s*    =\s* ".*";/col_gray1\[\]\       =\ "#1A1A1A";/g' dwm/config.h &&
	echo "Background color has changed"

## border color unfocused windows
sed -i 's/col_gray2\[\]\s*    =\s* ".*";/col_gray2\[\]\       =\ "#2B2B2B";/g' dwm/config.h &&
