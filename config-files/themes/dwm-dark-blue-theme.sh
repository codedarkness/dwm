...
# -----------------------------------------------------------------
#
#        FILE: dwm-dark-blue-theme.sh
#       USAGE: ./dwm-dark-blue-theme.sh
#
...
echo "Changing color scheme for dwm - Dark-Blue"

echo "-------------------------------------------------------------"

## background color
sed -i 's/col_gray1\[\]\s*    =\s* ".*";/col_gray1\[\]\       =\ "#0F111A";/g' dwm/config.h &&
	echo "Background color has changed"

## border color unfocused windows
sed -i 's/col_gray2\[\]\s*    =\s* ".*";/col_gray2\[\]\       =\ "#282a36";/g' dwm/config.h &&
	echo "Border color (unfocused) has changed"

## foreground color (font)
sed -i 's/col_gray3\[\]\s*    =\s* ".*";/col_gray3\[\]\       =\ "#96b5b4";/g' dwm/config.h &&
...
sed -i 's/col_cyan\[\]\s*     =\s* ".*";/col_cyan\[\]\        =\ "#34495E";/g' dwm/config.h &&
